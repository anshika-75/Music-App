class Artist::SongsController < ApplicationController
  ...
  def index
    # Redis Caching: Reads from or writes to cache under an artist-specific key
  ...
  def update
    ...
    if mp3_file.present?
      # Replacement Rule: If the MP3 file is changed, we save it as a new song record instead of editing this one
